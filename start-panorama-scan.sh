...
# sudo bash /home/pi/start-panorama-scan.sh &
#
...
dt=$(date '+%d-%m-%Y--%H:%M:%S')
echo $dt >> /home/pi/panorama-log.txt

if [ $online -eq 1 ]; then
  # if network down, then auto run our panorama-scan:
  echo "network down" >> /home/pi/panorama-log.txt
  /usr/bin/python3 /home/pi/panorama-scan.py /home/pi/panorama-images
else
  echo "network up" >> /home/pi/panorama-log.txt
fi
